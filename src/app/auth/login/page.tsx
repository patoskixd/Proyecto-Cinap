...
// Si ya tienes los casos de uso según tu arquitectura limpia, descomenta:
//import { useCases } from "@/presentation/di/useCases";
...
export default function LoginPage() {
 ...
 const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Limpio: dominio+caso de uso (Mock hoy, FastAPI/MCP mañana)
  ...
  const onGoogle = () => {
    // Aquí luego disparas tu flujo ConnectGoogle/NextAuth/Auth.js
    // Por ahora, solo de ejemplo:
    router.push("/auth/login"); // reemplaza por connectGoogle.execute(userId) → redirect authUrl
  };
  ...
  return (
    <section className="relative flex min-h-[calc(100vh-72px)] items-center justify-center overflow-hidden bg-[linear-gradient(135deg,#f8fafc_0%,#e2e8f0_100%)] px-4 py-6 md:py-10">
      {/* Shapes de fondo (aprox con animate-bounce y delays) */}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border-2 border-neutral-200 bg-white/80 px-12 py-3 text-[15px] outline-none transition focus:border-blue-600 focus:shadow-[0_0_0_3px_rgba(37,99,235,0.1)]"
                />
              </div>
            </div>
