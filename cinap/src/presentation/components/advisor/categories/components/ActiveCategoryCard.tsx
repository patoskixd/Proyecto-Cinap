...
export default function ActiveCategoryCard({
  ...
  const selectedServices = uniq.filter(s => s.selected).length;
  ...
  return (
    <div className="group relative overflow-hidden rounded-3xl border border-blue-200/40 bg-gradient-to-br from-white via-blue-50/30 to-yellow-50/20 shadow-lg backdrop-blur-sm transition-all duration-500 hover:-translate-y-2 hover:scale-105 hover:shadow-2xl hover:border-blue-300/60">
      ...
      <div className="absolute right-4 top-4 flex items-center gap-2">
        <div className="h-3 w-3 rounded-full bg-gradient-to-r from-green-400 to-green-500 shadow-lg animate-pulse" />
      </div>
      ...
      <div className="relative p-6">
        ...
        <div className="mb-5">
          ...
          {/* Status badge */}
          <div className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-green-100 to-green-200 px-3 py-1 text-xs font-semibold text-green-800 border border-green-300/50">
             Activa
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {chips.map((s) => (
                <span
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs font-medium border transition-all duration-200 hover:scale-105 ${
                    s.selected 
                      ? "border-yellow-300/70 bg-gradient-to-r from-yellow-100 to-yellow-200 text-yellow-800"
                      : "border-gray-300/70 bg-gradient-to-r from-gray-100 to-gray-200 text-gray-600"
                  }`}
                >
                  {s.name}
                </span>
              ...
              {hidden > 0 && (
                <span className="inline-flex items-center rounded-lg bg-gradient-to-r from-blue-100 to-blue-200 px-3 py-1.5 text-xs font-medium text-blue-700 border border-blue-300/70">
                  +{hidden} más
                </span>
              )}
            </div>
        ...
        <button
          ...
          className="group/btn w-full relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 via-blue-700 to-blue-800 px-4 py-3 font-semibold text-white shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-300"
        >
