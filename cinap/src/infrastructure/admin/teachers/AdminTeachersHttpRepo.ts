...
export class HttpTeachersRepo implements TeachersRepo {
  ...
  async list(): Promise<Teacher[]> {
    const res = await fetch(`${this.base}`, { method: "GET", cache: "no-store", credentials: "include" });
    if (!res.ok) throw new Error(await res.text() || "No se pudieron cargar los docentes");
    const data = await res.json();
    // backend devuelve {id, usuario_id, name, email, activo}; mapeamos a tu dominio
    return (data as any[]).map(t => ({ id: t.id, name: t.name, email: t.email }));
  }
